...
with global_top_tracks_weekly as (
    select
        * 
    from {{ source('spotify_source', 'global_top_tracks_weekly_stream')}}
    where synced_at = (select max(synced_at) from {{ source('spotify_source', 'global_top_tracks_weekly_stream')}})
),
artist_name_list_to_string as (
    {{ artist_array_pivot('global_top_tracks_weekly') }}
),
final as (
    select
        rank as rank
