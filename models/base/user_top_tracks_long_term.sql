...
with user_top_tracks_long_term as (
    select
        * 
    from {{ source('spotify_source', 'user_top_tracks_lt_stream')}}
    where synced_at = (select max(synced_at) from {{ source('spotify_source', 'user_top_tracks_lt_stream')}})
),
...
final as (
    ...
    from user_top_tracks_long_term utt
