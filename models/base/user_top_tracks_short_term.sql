...
with user_top_tracks_short_term as (
    select
        * 
    from {{ source('spotify_source', 'user_top_tracks_st_stream')}}
    where synced_at = (select max(synced_at) from {{ source('spotify_source', 'user_top_tracks_st_stream')}})
),
artist_name_list_to_string as (
    {{ artist_array_pivot('user_top_tracks_short_term') }}
),
final as (
    select
        rank as rank
